...
instance forall fs a. FromDbValues fs => ParseOutput (Delete One (Just fs) a) where
...
instance forall fs a. FromDbValues fs => ParseOutput (Delete Many (Just fs) a) where
...
instance forall fs a. FromDbValues fs => ParseOutput (Insert One (Just fs) a) where
...
instance forall fs a. FromDbValues fs => ParseOutput (Insert Many (Just fs) a) where
  type OutputT (Insert Many (Just fs) a) = [fs]
  parse (OutputRows rows)                = Right $ fromDbValues <$> rows
  parse output                           = Left  $ ExpectedOutputRows output

instance forall fs a ob. FromDbValues fs => ParseOutput (Select One fs a ob) where
  type OutputT (Select One fs a ob) = Maybe fs
  parse (OutputRows [])             = Right Nothing
  parse (OutputRows [row])          = Right $ Just $ fromDbValues row
  parse output                      = Left  $ ExpectedMaybeOne output

instance forall fs a ob. FromDbValues fs => ParseOutput (Select Many fs a ob) where
